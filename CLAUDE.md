...
- `auth/` — signup, login, logout, 30-day session tokens, **account deletion** (`DELETE /auth/me`, RequireAuth): password-verified **hard delete** (`repo.DeleteUser` → `DELETE FROM app_users`, FK-cascades everything off the user; ADR-0008) with best-effort GCS image cleanup wired nil-safe via `WithImageGC` (the `imagegc` adapter collects the user's own-bucket image keys from `wishlists`/`wishlist_items` **before** the delete via `GCSUploader.KeyFromPublicURL` — the single reverse of `publicURL`, so the URL→key mapping can't drift — and removes the objects **after**, so a failed delete never orphans). The post-delete removal is **detached** off the request path in api/all via `WithImageGCAsync` (mirrors the notification-push split; sync in tests for deterministic ordering). **`birthday` is optional** everywhere (nullable DB col, `*time.Time` domain/DTOs, no `IsZero` validator) — not asked at signup, **editable and clearable** later in the Account screen (the field shows a clear affordance once a birthday is set).
