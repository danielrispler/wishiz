...
void main() {
  ...
  group('SharedPreferencesWishlistStorage', () {
    ...
    test('clears all legacy wishlist keys during startup cleanup', () async {
      SharedPreferences.setMockInitialValues({});
      final firstUserStorage = await SharedPreferencesWishlistStorage.create(
        userId: 'user-1',
      );
      ...
      await firstUserStorage.write('first-user-lists');
      await secondUserStorage.write('second-user-lists');
      await preferences.setString('wishiz.wishlists', 'legacy');

      await SharedPreferencesWishlistStorage.clearLegacyData();

      expect(await firstUserStorage.read(), isNull);
      expect(await secondUserStorage.read(), isNull);
      expect(preferences.getString('wishiz.wishlists'), isNull);
    });
  });
}
